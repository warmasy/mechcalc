...
from mechcalc.core.units import Q_, to_mag


class TestSolidCylinderTensor:
    def test_izz_matches_cylinder(self):
        """互锁：张量 Izz == cylinder().J_y（绕对称轴）"""
        I = np.asarray(solid_cylinder_tensor(10, 100, 200).magnitude)
        J = to_mag(cylinder(10, 100, 200)["J_y"], "kg*m**2")
...
class TestHollowCylinderTensor:
    def test_izz_matches_tube(self):
        """互锁：张量 Izz == tube().J_y（绕对称轴）"""
        I = np.asarray(hollow_cylinder_tensor(10, 100, 50, 200).magnitude)
        J = to_mag(tube(10, 100, 50, 200)["J_y"], "kg*m**2")
        assert abs(I[2, 2] - J) < 1e-12
...
class TestParallelAxisTensor:
    def test_shift_then_about_z(self):
        ...
        J_c = to_mag(cylinder(10, 100, 200)["J_y"], "kg*m**2")
...
class TestInertiaAboutAxis:
    def test_z_axis_matches_scalar(self):
        ...
        assert abs(to_mag(J, "kg*m**2") - to_mag(cylinder(10, 100, 200)["J_y"], "kg*m**2")) < 1e-12
