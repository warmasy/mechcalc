...
# 标准缸径表（单位: mm）
STD_BORES: list[int] = [6, 8, 10, 12, 16, 20, 25, 32, 40, 50, 63, 80, 100, 125, 160, 200, 250, 320]
...
def pull_force(
    ...
    D = set_quantity(D, "mm")
    if d is None:
        d = Q_(max(round(float(D.magnitude) * ROD_DIA_RATIO), ROD_DIA_MIN), "mm")
    else:
        d = set_quantity(d, "mm")
    eta = float(efficiency)
...
def air_consumption(
    ...
    Q = vol_std * set_quantity(cycles_per_min, "1/min")
...
def cylinder_select(
    ...
    D_theory = set_quantity(D_theory_mm, "mm")

    selected_bore: int | None = None
    ...
    d_rod = Q_(max(round(selected_bore * ROD_DIA_RATIO), ROD_DIA_MIN), "mm")
    raw_pull = pull_force(P, selected_bore, d=d_rod, efficiency=eta)
    F_pull = raw_pull["F"]

    return {
        "theoretical_bore": D_theory.to("mm"),
        "selected_bore": set_quantity(selected_bore, "mm"),
